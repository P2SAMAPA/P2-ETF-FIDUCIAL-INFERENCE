...
st.set_page_config(page_title="Fiducial Inference – Generalized", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Fiducial Inference – Generalized</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Fisherian fiducial inference | Third way between frequentist and Bayesian | Non-identifiable parameters</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Fiducial")
...
st.sidebar.markdown(f"**Samples:** {config.N_SAMPLES}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fiducial_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Fiducial confidence: {etf['fiducial_score_norm']:.3f}</p>
...
with tab1:
    st.header("📊 Top ETFs by Fiducial Confidence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fiducial inference** (Fisher) is a third way between frequentist and Bayesian statistics.
        - The **generalized fiducial distribution** provides confidence in parameter estimates.
        - The score is the **fiducial confidence** that the ETF's expected return is positive.
        - Higher confidence → stronger signal for long position.
        - Particularly useful for models with non-identifiable parameters.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the fiducial confidence scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fiducial Inference | Generalized fiducial inference for parameter confidence")
